...
function addExportDirectives(operation, rawVariableValues) {
  ...
  var exportedVariables = getExportedVariablesAndFields(selections, errors);
  ...
  for (var _i4 = 0; _i4 < variableDefinitions.length; _i4++) {
    ...
    if (exports[v] !== undefined) {
      if (!isScalarType(v)) {
        errors.push(new GraphQLError("Exported variable \"".concat(v, "\" is not a scalar type")));
      }
    ...
    if (selection.selectionSet === undefined) {
      for (var _i8 = 0, _selection$directives2 = selection.directives; _i8 < _selection$directives2.length; _i8++) {
        ...
        if (directive.name.value === 'export') {
          var v = directive.arguments[0].value.value; // don't allow exports from nested fields

          if (level > 1) {
            errors.push(new GraphQLError("Variable \"".concat(v, "\" cannot be exported from a nested field")));
            return exports;
          }

          if (exports[v] !== undefined) {
            errors.push(new GraphQLError("Exported variable \"".concat(v, "\" can only be exported once")));
          }

          exports[v] = [selection, directive];
        }
      }
    } else {
      getExportedVariablesAndFields(selection.selectionSet.selections, errors, level + 1, exports);
    }
  }
...
function executeFieldsSerially(exeContext, parentType, sourceValue, path, fields) {
  ...
  exeContext.contextValue.responseFields = Object.keys(fields); // collect results

  var promisedResults = [];

  for (var responseName in fields) {
  ...
  return promiseReduce(Object.keys(fields), function (results, responseName) {
    var result = promisedResults[responseName];
